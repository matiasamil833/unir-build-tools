JENKINS_DOCKER_AGENT_SECRET := 337f4e3525f5d55a7163ba18be58a5d3d99d598432f4292d6b31f0c13442f51b
JENKINS_MAVEN_AGENT_SECRET := 3d5384331af21089ab32fb6d4299b3d7881b0df8e31f461fb5ad691d9110ea5c
JENKINS_NODE_AGENT_SECRET := 49ffaf82820831b2d590333f82a3028cc47f75c004ee184662d89f5b04e71f8e
...
start-nexus:
	docker run -d --name nexus-server -v nexus-data:/nexus-data -p 8081:8081 -p 8082:8082 sonatype/nexus3

start-nexus-jenkins:
	docker run -d --rm --network jenkins --name nexus-server -v nexus-data:/nexus-data -p 8081:8081 -p 8082:8082 sonatype/nexus3
...
start-sonarqube-jenkins:
	docker run -d --rm --network jenkins --network-alias sonarqube-server --name sonarqube-server -p 9000:9000 sonarqube:community
 
stop-sonarqube:
	docker stop --time=120 sonarqube-server